...
describe("Selection", () => {
  ...
  describe("distance calculation", () => {
    ...
    describe("point to line distance", () => {
      it("should calculate distance from point to horizontal line", () => {
        ...
        const distance = selection.calculateDistanceToLine(
          { x: 100, y: 50 },
          line,
        );
      ...
      it("should calculate distance to line segment endpoints", () => {
        ...
        const distanceLeft = selection.calculateDistanceToLine(
          { x: 0, y: 50 },
          line,
        );
        ...
        const distanceRight = selection.calculateDistanceToLine(
          { x: 200, y: 50 },
          line,
        );
    ...
    describe("point to arc distance", () => {
      it("should calculate distance from point to arc center", () => {
        ...
        const distance = selection.calculateDistanceToArc(
          { x: 100, y: 50 },
          arc,
        );
      ...
      it("should calculate distance from point outside arc to arc boundary", () => {
        ...
        expect(distance).toBeCloseTo(111.8, 1); // Distance to closest arc endpoint
      });
    ...
    describe("arc closest point", () => {
      ...
      it("should return arc endpoint when point is outside angle range", () => {
        ...
        const distToStart = Math.sqrt(
          (closestPoint!.x - 150) ** 2 + (closestPoint!.y - 100) ** 2,
        );
        const distToEnd = Math.sqrt(
          (closestPoint!.x - 100) ** 2 + (closestPoint!.y - 150) ** 2,
        );
  ...
  describe("arc angle range selection", () => {
    ...
    it("should select partial arc only when clicking within angle range", () => {
      ...
      const distanceWithin = selection.calculateDistanceToArc(
        { x: 135, y: 135 },
        arc,
      );
      ...
      const distanceOutside = selection.calculateDistanceToArc(
        { x: 50, y: 100 },
        arc,
      );
