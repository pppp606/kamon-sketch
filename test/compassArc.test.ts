...
describe("CompassArc", () => {
  ...
  describe("radius calculation", () => {
    ...
    it("should return currentRadius if radius point is not set", () => {
      compassArc.setCenter(100, 100);
      expect(compassArc.getRadius()).toBe(10); // DEFAULT_RADIUS
    });

    it("should return currentRadius if center point is not set", () => {
      compassArc = new CompassArc();
      expect(compassArc.getRadius()).toBe(10); // DEFAULT_RADIUS
    });
  ...
  describe("radius persistence", () => {
    ...
    it("should initialize with default radius when no stored value exists", () => {
      compassArc = new CompassArc();
      expect(compassArc.getCurrentRadius()).toBe(10);
    });

    it("should load radius from localStorage on initialization", () => {
      mockStorage["compass.radius"] = "25";
      compassArc = new CompassArc();
      expect(compassArc.getCurrentRadius()).toBe(25);
    });

    it("should clamp loaded radius to valid range", () => {
      mockStorage["compass.radius"] = "0.5"; // Below minimum
      compassArc = new CompassArc();
      expect(compassArc.getCurrentRadius()).toBe(1); // MIN_PERSISTENT_RADIUS

      mockStorage["compass.radius"] = "15000"; // Above maximum
      compassArc = new CompassArc();
      expect(compassArc.getCurrentRadius()).toBe(10000); // MAX_PERSISTENT_RADIUS
    });

    it("should ignore invalid stored radius values", () => {
      mockStorage["compass.radius"] = "invalid";
      compassArc = new CompassArc();
      expect(compassArc.getCurrentRadius()).toBe(10); // DEFAULT_RADIUS
    });

    it("should set current radius with clamping", () => {
      compassArc = new CompassArc();
      compassArc.setCurrentRadius(50);
      expect(compassArc.getCurrentRadius()).toBe(50);
      expect(mockStorage["compass.radius"]).toBe("50");
    });

    it("should clamp radius values in setCurrentRadius", () => {
      compassArc = new CompassArc();
      compassArc.setCurrentRadius(0.5);
      expect(compassArc.getCurrentRadius()).toBe(1); // MIN_PERSISTENT_RADIUS

      compassArc.setCurrentRadius(15000);
      expect(compassArc.getCurrentRadius()).toBe(10000); // MAX_PERSISTENT_RADIUS
    });

    it("should preserve last radius when setting new radius", () => {
      compassArc = new CompassArc();
      expect(compassArc.getCurrentRadius()).toBe(10); // Initial default

      compassArc.setCurrentRadius(25);
      expect(compassArc.getCurrentRadius()).toBe(25);

      compassArc.setCurrentRadius(50);
      expect(compassArc.getCurrentRadius()).toBe(50);

      // Check that last radius was preserved (should be 25)
      compassArc.useLastRadius();
      expect(compassArc.getCurrentRadius()).toBe(25);
    });

    it("should swap current and last radius with useLastRadius", () => {
      compassArc = new CompassArc();
      compassArc.setCurrentRadius(25);
      compassArc.setCurrentRadius(50);
      ...
      compassArc.useLastRadius();
      expect(compassArc.getCurrentRadius()).toBe(25);
      expect(mockStorage["compass.radius"]).toBe("25");
    ...
    it("should save radius to localStorage when changed", () => {
      compassArc = new CompassArc();
      compassArc.setCurrentRadius(75);
      expect(mockStorage["compass.radius"]).toBe("75");

      compassArc.useLastRadius();
      expect(mockStorage["compass.radius"]).toBe("10"); // Original default
    });
    ...
    it("should handle localStorage errors gracefully", () => {
      ...
      // Should not throw errors and should use default values
      expect(() => {
        compassArc = new CompassArc();
        compassArc.setCurrentRadius(25);
        compassArc.useLastRadius();
      ...
      expect(compassArc.getCurrentRadius()).toBe(10); // Should still work with defaults
    });

    it("should handle missing localStorage (server-side environment)", () => {
      // Remove localStorage
      ...
      expect(compassArc.getCurrentRadius()).toBe(10); // Should work with defaults
    });
  });

  describe("static methods", () => {
    it("should expose radius constants", () => {
      expect(CompassArc.getDefaultRadius()).toBe(10);
      expect(CompassArc.getMinPersistentRadius()).toBe(1);
      expect(CompassArc.getMaxPersistentRadius()).toBe(10000);
      expect(CompassArc.getRadiusStorageKey()).toBe("compass.radius");
    });

    it("should expose existing constants", () => {
      expect(CompassArc.getMinRadius()).toBe(0.001);
      expect(CompassArc.getFullCircleThreshold()).toBe(0.1);
    });
  });
});
