  ...
  if (drawingMode === "compass" && compassArc) {
    ...
    if (state === "IDLE") {
      ...
      if (isShiftPressed || isInShiftRadiusMode) {
        ...
        const startAngle = compassArc.calculateStartAngleFromClick(
          p.mouseX,
          p.mouseY,
        );
        // Set radius at angle and immediately start drawing to avoid RADIUS_SET state
        compassArc.setRadiusAtAngle(startAngle, currentRadius, true);
        compassArc.updateDrawing(p.mouseX, p.mouseY);
      }
      return;
    }
  ...
  if (closestElement) {
    ...
    if (distance <= SELECTION_THRESHOLD) {
      ...
      if (closestElement.type === "line") {
        ...
        compassRadiusState.setRadiusFromShape(
          closestElement.element as CompassArc,
        );
      }
