...
export class Fill {
  ...
  private colorsMatch(r1: number, g1: number, b1: number, r2: number, g2: number, b2: number): boolean {
  ...
  floodFill(p: P5Instance, startX: number, startY: number): FillResult {
    ...
    if (this.colorsMatch(startR, startG, startB, this.fillColor.r, this.fillColor.g, this.fillColor.b)) {
      return { pixelCount: 0 }
    }
    ...
    while (stack.length > 0) {
      ...
      if (current.x < 0 || current.x >= p.width || current.y < 0 || current.y >= p.height) {
        continue
      }
      ...
      if (!this.colorsMatch(currentR, currentG, currentB, startR, startG, startB)) {
        continue
      }
  ...
  isPointInRegion(p: P5Instance, x: number, y: number): boolean {
    ...
    return this.colorsMatch(pixelColor[0] ?? 255, pixelColor[1] ?? 255, pixelColor[2] ?? 255, 255, 255, 255)
  }
