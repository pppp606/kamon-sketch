...
export class Selection {
  ...
  isSelected(element: SelectableElement): boolean {
    ...
    return (
      this.selectedElement.type === element.type &&
  ...
  calculateDistanceToArc(point: Point, arc: CompassArc): number {
    ...
    if (
      !centerPoint ||
      (arc.getState() !== "DRAWING" && arc.getState() !== "RADIUS_SET")
    ) {
      return Infinity;
    }
    ...
    const pointAngle = this.normalizeAngle(
    ...
    if (this.isAngleInRange(pointAngle, startAngle, endAngle)) {
      ...
      const distToStart = this.calculateDistance(point, {
        x: startPointX,
        y: startPointY,
      });
      const distToEnd = this.calculateDistance(point, {
        x: endPointX,
        y: endPointY,
      });
